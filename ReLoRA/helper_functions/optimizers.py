...
def create_optimizer_adamW(model, learning_rate, args):
    optimizer = torch.optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=learning_rate, weight_decay=args.weight_decay, betas=(0.9,0.999))  # true wd, filter_bias_and_bn
    return optimizer
