...
from helper_functions.dora_instantiate import instantiate_relora_model, toggle_base_model
...
def distributed_model_reset(old_model=None, old_optimizer=None, old_scheduler=None, old_scaler=None):
    for obj in [old_model, old_optimizer, old_scheduler, old_scaler]:
        try:
            del obj
        except:
            pass
...
def train_21k(args, model, train_loader, val_loader, world_size, global_rank, local_rank):
    ...
    is_conv_warmed_up = all_vars[9]
    has_convergence_passed = all_vars[10]

    switch_model_start_iter = all_vars[11]
    #############################################################################################################
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, (input, target) in enumerate(train_loader):
            ...
            if int(current_effective_iteration) == int(args.relora_switch_step) and not args.has_lora:
                ...
                del output, loss, input, target
                gc.collect()
                torch.cuda.empty_cache()

                base_model = model.module  # extract the underlying nn.Module
                del model
                del optimizer
                del scheduler
                del scaler
                ...
                model = get_peft_model(base_model, peft_config).to(local_rank)
                ...
                scheduler.step(current_effective_iteration)
                scaler = GradScaler(device='cuda')

                if global_rank == 0:
                    save_model(args, model, optimizer, scheduler, epoch, iteration_counter,
                            has_convergence_passed, True, stored_k1_losses, stored_k1_weight_norms, k1_total_loss, switch_model_start_iter)
            ...
            if should_switch.item() == 1 and not has_convergence_passed:                
                ...
                switch_model_start_iter = current_effective_iteration
                
                if global_rank == 0:
                    save_model(args, model, optimizer, scheduler, epoch, iteration_counter, has_convergence_passed, is_model_warmed_up, stored_k1_losses, stored_k1_weight_norms, k1_total_loss, switch_model_start_iter)
                    
                optimizer.zero_grad(set_to_none=True)

                print_at_master(f"Reached here at:{current_effective_iteration}")
                model.module.print_trainable_parameters()

                torch.cuda.empty_cache()
                torch.cuda.synchronize()

            if has_convergence_passed and switch_model_start_iter is not None and (int(current_effective_iteration) - switch_model_start_iter) > args.warmup_steps:
                model, optimizer, scheduler = toggle_base_model(model.module, scheduler, args, local_rank, 1.28e6, True, 0.1)
                
                print(f"Toggled model finally at:{current_effective_iteration}")
                model.module.print_trainable_parameters()
                ...
                if ((epoch> 0 and epoch% 5 == 0) or epoch == (args.epochs-1)) and global_rank == 0:
                    if hasattr(scheduler, 'original_scheduler'):
                        save_sch = scheduler.original_scheduler
                    else:
                        save_sch = scheduler

                    save_model(args, model, optimizer, save_sch, epoch, iteration_counter, has_convergence_passed, is_model_warmed_up, stored_k1_losses, stored_k1_weight_norms, k1_total_loss, switch_model_start_iter)
                    sys.exit()
                
            if is_model_warmed_up and (int(current_effective_iteration) - args.relora_switch_step) > 0 and (int(current_effective_iteration) - args.relora_switch_step)% args.merge_and_reinit_step == 0 and not has_convergence_passed:
                ...
                model = merge_and_reinit_model(base_model)
                model = model.to(local_rank)

                model = to_ddp(model, args, local_rank)

                lora_params = [p for n, p in model.module.named_parameters() if p.requires_grad and "lora_" in n]
                optimizer_reset(optimizer, lora_params, optimizer_state_keys, 0.75 if has_convergence_passed else 0.25)
                scheduler = WarmupOverrideScheduler(optimizer, scheduler, warmup_steps=200)
                scheduler.start_warmup()
                scaler = GradScaler(device='cuda')

                print_at_master(f"After lora reset at update step {current_effective_iteration}. After lr is {optimizer.param_groups[0]['lr']}")
