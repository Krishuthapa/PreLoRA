...
def train_21k(args, model, train_loader, val_loader, world_size, global_rank, local_rank):
    ...
    is_switching_to_small_model = False
    #############################################################################################################
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, (input, target) in enumerate(train_loader):
            ...
            #                 has_convergence_passed, True, stored_k1_losses, stored_k1_weight_norms, k1_total_loss)
            ...
            #     is_switching_to_small_model = True
            ...
            #     print_at_master("ya pugda")
            #     torch.cuda.empty_cache()
            #     torch.cuda.synchronize()
                
            # if is_model_warmed_up and (int(current_effective_iteration) - args.relora_switch_step) > 0 and ((int(current_effective_iteration) - args.relora_switch_step) % (4 * args.merge_and_reinit_step) == 0 if has_convergence_passed else (int(current_effective_iteration) - args.relora_switch_step) % args.merge_and_reinit_step == 0) and (i + 1) % args.grad_accum_steps == 0 and not is_switching_to_small_model:
            # if is_model_warmed_up and (int(current_effective_iteration) - args.relora_switch_step) > 0 and  not has_convergence_passed and (int(current_effective_iteration) - args.relora_switch_step) % args.merge_and_reinit_step == 0 and (i + 1) % args.grad_accum_steps == 0 and not is_switching_to_small_model:
            ...
            if (i + 1) % args.grad_accum_steps == 0:
                ...
                # if is_switching_to_small_model:
                #     is_switching_to_small_model = False
