...
from model.vitmodel import ViT

from dataloader.data_loader import create_data_loaders
from helper_functions.distributed import print_at_master, to_ddp, reduce_tensor, num_distrib, setup_distrib
from helper_functions.general_helper_functions import accuracy, AverageMeter, silence_PIL_warnings

from helper_functions.optimizers import create_optimizer, create_optimizer_sgd
...
def main():
    ...
    model = ViT(image_size = 224, patch_size = 16, dim = 1024, depth=24, heads = 16, dim_head = 64, mlp_dim = 4096, num_classes = args.num_classes).cuda()
...
def train_21k(model, train_loader, val_loader, optimizer, args):
    ...
    scheduler = lr_scheduler.OneCycleLR(optimizer, max_lr=args.lr, steps_per_epoch=len(train_loader),
                                        epochs=args.epochs, pct_start=0.15, cycle_momentum=False, div_factor=20)
